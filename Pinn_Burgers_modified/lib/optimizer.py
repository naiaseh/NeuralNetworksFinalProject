...
#     def __init__(self, model, x_train, y_train, factr=1e7, m=50, maxls=50, maxiter=5000,epochs=1):
#         """
#         Args:
#             model: optimization target model.
#             samples: training samples.
...
#             count_mode='steps', stateful_metrics=self.metrics)
#         self.progbar.set_params( {
#             'verbose':1, 'epochs':self.epochs, 'steps':self.maxiter, 'metrics':self.metrics, 'loss':self.loss})
...
#         with tf.GradientTape() as g:
#             self.loss = tf.reduce_mean(tf.keras.losses.mse(self.model(x), y))
#         grads = g.gradient(self.loss, self.model.trainable_variables)
...
#         self.set_weights(weights)
#         # compute loss and gradients for weights
#         self.loss, grads = self.tf_evaluate(self.x_train, self.y_train)
#         # convert tf.Tensor to flatten ndarray
#         self.loss = self.loss.numpy().astype('float64')
#         grads = np.concatenate([ g.numpy().flatten() for g in grads ]).astype('float64')
...
#         """
#         self.progbar.on_batch_begin(0)
#         self.loss, _ = self.evaluate(weights)
#         self.progbar.on_batch_end(0, logs=dict(zip(self.metrics, [self.loss])))
